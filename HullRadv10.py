...
def model_from_pdb(file):
    ...
    if backbone_atms_check != 0:
        ...
        raise ValueError(' Backbone atom missing.')
        ...
        if (line[13:15] == 'N ' or \
          ...
          line[13:16] == 'OT1'):
            ...
            if (math.fmod(line_counter,4) == 0 and line[17:20] != 'GLY' \
                ...
                raise ValueError(' Found non-gly residue with no CB atom')
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=args.num_workers) as executor:
        results = list(tqdm(executor.map(process_file, file_paths), total=len(file_paths), desc="Processing PDBs"))

    results = [r for r in results if r is not None]

    df = pl.DataFrame(results)

    output_csv = f"{args.output_csv}.tsv"
    df.write_csv(output_csv, separator="\t")
    
    end = time.perf_counter()
    duration = end - start
    print(f"Done. Results written to {output_csv} in {duration:.2f} seconds.")
